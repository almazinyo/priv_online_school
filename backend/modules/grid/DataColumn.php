...
class DataColumn extends YiiDataColumn
{
    ...
    protected function renderFilterCellContent()
    {
        ...
        if ($this->filterType === GridView::FILTER_SELECT2 && empty($this->filter)) {

//            $options['data'] = ArrayHelper::map($this->grid->dataProvider->getModels(), "title", "title");

            $model = new $this->grid->filterModel;
            $options['data'] = ArrayHelper::map($model::find()->select($this->attribute)->asArray()->all(), $this->attribute, $this->attribute);
            $options['options']['placeholder'] = Inflector::camel2words($this->attribute, true);
        }
