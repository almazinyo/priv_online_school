...
use backend\modules\grid\assets\GridExportAsset;
use backend\modules\grid\assets\GridFloatHeadAsset;
use backend\modules\grid\assets\GridPerfectScrollbarAsset;
use backend\modules\grid\assets\GridResizeColumnsAsset;
use backend\modules\grid\assets\GridResizeStoreAsset;
use backend\modules\grid\assets\GridToggleDataAsset;
use backend\modules\grid\assets\GridViewAsset;
use backend\modules\grid\models\FieldSettings;
 ...
 *
 * The GridView is available and configurable as part of the Krajee grid [[ModuleOld]] with various new additional grid
...
HTML;
     ...
     * - `label`: _string_, the label fkh or the action button. Defaults to empty string.
    ...
    public function init()
    {
        ...
        $this->custom_columns = $this->init_custom_columns($columns);

        $class_name = $this->className = $dataProvider->query->modelClass;

        $gridModel = $this->model = GridSort::find()
            ->where([
        ...
        if (empty($gridModel)) {
            $gridModel = $this->model = new GridSort();
            $gridModel->user_id = Yii::$app->user->id;
            $gridModel->class_name = $class_name; /*!empty($filterModel) ? $filterModel : $this->className;*/
            $gridModel->label = Yii::$app->view->title;
            $gridModel->save(false);
        } else {

            if (!empty($gridModel['visible_columns'])) {
                $this->visible_columns = json_decode($gridModel['visible_columns'], true);
            }

            /**
             * change page size
             */
            $dataProvider->getPagination()->setPageSize(empty($gridModel['page_size']) ? 20 : $gridModel['page_size']);
        }

        if (!empty($dataProvider->getModels()[0]->attributes)) {
            $allColumnNames = array_keys($dataProvider->getModels()[0]->attributes);
            $this->allColumnNames = array_combine($allColumnNames, $allColumnNames);
        }
    ...
    public function renderPageSummary()
    {
        ...
        foreach ($this->columns as $column) {
            /* @var $column \backend\modules\grid\ColumnTrait */
    ...
    protected static function parseExportConfig($exportConfig, $defaultExportConfig)
    {
        if (is_array($exportConfig) && !empty($exportConfig)) {
            foreach ($exportConfig as $format => $setting) {
                ...
                $exportConfig[$format] = empty($setup)
    ...
    protected function getVisibleAttributes()
    {
        ...
        if (!empty($this->visible_columns)) {
            ...
            $visibleAttributes = $this->init_custom_columns($this->visible_columns);;
        } elseif (!empty($this->model['default_columns'])) {
            $visibleAttributes = json_decode($this->model['default_columns']);
        } elseif (!empty($this->custom_columns)) {
            $visibleAttributes = $this->custom_columns;
        } else {
            $visibleAttributes = $this->allColumnNames;
        }

        $temp = [];

        foreach ($visibleAttributes as $val) {
            if ($this->isValidFiledSettings) {
                $temp[] = [
                    "content" => "<span class='column' >$val</span>
 <span class='field_settings'>
<a data-toggle='modal' href='#field_settings'  data-column-name='$val' data-class-name='$this->className' >
        <i class='fa fa-cogs' aria-hidden='true'></i>
</a>
</span>",
                ];
            } else {
                $temp[] = ["content" => "<span class='column' >$val</span>"];
            }
        }
        return $temp;
    }
    ...
    protected function getHiddenAttributes()
    {
        ...
        if (!empty($this->visible_columns) && !empty($this->allColumnNames)) {
            $hiddenAttributes = array_diff($this->allColumnNames, $this->init_custom_columns($this->visible_columns));
        } else {
            if (!empty($this->model['default_columns'])) {
                $hiddenAttributes = array_diff($this->allColumnNames, json_decode($this->model['default_columns']));
            } elseif (!empty($this->custom_columns) && !empty($this->allColumnNames)) {
                $hiddenAttributes = array_diff($this->allColumnNames, $this->custom_columns);
            }
        }
        $temp = [];

        foreach ($hiddenAttributes as $val) {
            $temp[] = ["content" => "<span class='column' >$val</span>"];
        }
        return $temp;
    }
    ...
    protected function guessColumns()
    {
        if (empty($this->model['default_columns'])) {
            parent::guessColumns();
//            $this->columns = array_slice($this->columns, 0, 4);

        } else {
            $this->columns = json_decode($this->model['default_columns']);
        }

        if ($this->isValidSerialColumn) {
            array_unshift($this->columns, ['class' => 'yii\grid\SerialColumn']);
        }
        if ($this->isValidActionColumn) {
            array_push($this->columns, ['class' => 'yii\grid\ActionColumn']);
        }
    }
    ...
    protected function initColumns()
    {
        ...
        if (!empty($visible_columns)) {
            $this->init_custom_columns($this->columns, $visible_columns);
    ...
    protected function initExport()
    {
        ...
        $this->exportConversions = array_replace_recursive(
            [
                ['from' => self::ICON_ACTIVE, 'to' => Yii::t('app', 'Active')],
                ['from' => self::ICON_INACTIVE, 'to' => Yii::t('app', 'Inactive')],
            ],
        ...
        $this->export = array_replace_recursive(
            [
                ...
                'options' => ['class' => 'btn btn-default', 'title' => Yii::t('app', 'Export')],
        ...
        if (!isset($this->export['header'])) {
            $this->export['header'] = '<li role="presentation" class="dropdown-header">' .
                Yii::t('app', 'Export Page Data') . '</li>';
        }
        if (!isset($this->export['headerAll'])) {
            $this->export['headerAll'] = '<li role="presentation" class="dropdown-header">' .
                Yii::t('app', 'Export All Data') . '</li>';
        }
        $title = empty($this->caption) ? Yii::t('app', 'Grid Export') : $this->caption;
        $pdfHeader = [
            'L' => [
                'content' => Yii::t('app', 'Yii2 Grid Export (PDF)'),
                'font-size' => 8,
                'color' => '#333333',
            ],
            'C' => [
        ...
        $pdfFooter = [
            'L' => [
                'content' => Yii::t('app', "© Krajee Yii2 Extensions"),
                'font-size' => 8,
                'font-style' => 'B',
                'color' => '#999999',
            ],
    ...
    protected function initLayout()
    {
        ...
        if (strpos($this->layout, '{setting}}') > 0) {
    ...
    protected function renderPanel()
    {
        ...
        if (!$this->bootstrap || !is_array($panel) || empty($panel)) {
            $panel = [
                'type' => empty($model['theme']) ? 'primary' : $model['theme'],
            ];
//            return;
        }
        ...
        $beforeOptions = ArrayHelper::getValue($panel, 'beforeOptions', []);
        $afterOptions = ArrayHelper::getValue($panel, 'afterOptions', []);
        $panelHeading = '';
        $panelBefore = '';
        $panelAfter = '';
        $panelFooter = '';
    ...
    protected function renderSetting()
    {
        ...
        return $this->render(
            "options/settings", [
            'model' => $this->model,
            'modelFieldSettings' => new FieldSettings(),
            'footer' => $this->renderActionButton('reset') . $this->renderActionButton('submit'),
            /* for sortable*/
...
//                'id' => $this->_gridModalId,
    ...
    protected static function getDefaultButtonOptions($type)
    {
        if ($type === 'submit') {
            return [
                ...
                'title' => Yii::t('app', 'Save grid settings'),
        ...
        if ($type === 'reset') {
            return [
                ...
                'title' => Yii::t('app', 'Abort any changes and reset settings'),
                'class' => 'btn btn-default',
                'data-pjax' => false,
            ];
        }
        if ($type === 'delete') {
            return [
                ...
                'title' => Yii::t('app', 'Remove saved grid settings'),
    ...
    protected function renderActionButton($type)
    {
        ...
        $label .= ArrayHelper::remove($options, 'label', '');
        Html::addCssClass($options, "grid-{$type}");
        return Html::button($label, $options);
    }

    private function init_custom_columns($columns, &$changeData = [])
    {
        if (empty($columns)) {
            return [];
        }
        ...
        foreach ($columns as $val) {
            if (!empty($changeData)) {
                if (!is_array($val)) {
                    if (strpos($val, ":")) {
                        $temp = explode(":", $val);
                        $searchKey = array_search($temp[0], $changeData);
                        if ($searchKey != false) {
                            $changeData[$searchKey] =
                                [
                                    'attribute' => $temp[0],
                                    'format' => $temp[1],
                                ];
                    ...
                    if (!empty($val['attribute'])) {
                        $searchKey = array_search($val['attribute'], $changeData);

                        if ($searchKey !== false) {
                            $changeData[$searchKey] = $val;
                        }
                    }

                    if (!empty($val['class']) && strpos($val['class'], 'ActionColumn') !== false) {
                        $this->isValidActionColumn = false;
                        $changeData[] = $val;
                    }
                }
            } else {
